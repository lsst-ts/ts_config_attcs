...
pipeline {
    ...
    environment {
        // Python module name.
        MODULE_NAME = 'lsst.ts.config.attcs'
            ...
            steps {
                ...
                withEnv(["HOME=${env.WORKSPACE}"]) {
                    sh """
                        ...
                        # ts_ATDome does not use ts_tcpip yet,
                        # but is likely to in the future
                        update_package ts_tcpip
                        update_package ts_observatory_control

                        # Update CSC packages
                        update_or_clone_package ts_ataos
                        update_or_clone_package ts_ATDome
                        update_or_clone_package ts_ATDomeTrajectory
                        update_or_clone_package ts_athexapod
                    """
                }
            }
        }
